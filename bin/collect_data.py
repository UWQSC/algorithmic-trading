...
def get_combined_sp_tickers():
    ...
    try:
        ...
        print("Found {} unique tickers".format(len(all_tickers)))
        
        return all_tickers
    except (ValueError, IndexError) as error:
        print("Error fetching tickers: {}".format(error))
        return []


def get_stock_data(ticker_file='sp_ticker.csv', output_csv='combined_stock_data.csv', 
                   output_parquet='combined_stock_data.parquet'):
    """
...
if __name__ == '__main__':
    ...
    # Create data directory path (one level up from script location)
    ...
    TICKERS_DF = pd.DataFrame(SP_TICKERS, columns=['Ticker'])
